...
app.get('/clientes', (req, res) => {
  const { nombre, estado = 'activo' } = req.query;  // Obtener nombre y estado de los parámetros de consulta
  let query = `
      ...
      WHERE c.estado = ?
  `;
  const queryParams = [estado];  // Parámetro para el estado

  // Si se proporciona el parámetro de nombre, añade el filtro de nombre
  if (nombre) {
...
app.get('/pedidos', (req, res) => {
  const { nombre, estado, fechaFiltro } = req.query;
...
if (fechaFiltro) {
  ...
  const todayDate = `${year}-${month}-${day}`; // Formato YYYY-MM-DD

  if (fechaFiltro === "hoy") {
      query += ` AND DATE(p.fecha_de_pedido) = ?`;
      params.push(todayDate);
  } else if (fechaFiltro === "7dias") {
      const pastDate = new Date(today);
      pastDate.setDate(today.getDate() - 7);
      const past7Days = pastDate.toISOString().split("T")[0];
      
      query += ` AND DATE(p.fecha_de_pedido) BETWEEN ? AND ?`;
      params.push(past7Days, todayDate);
  } else if (fechaFiltro === "30dias") {
      const pastDate = new Date(today);
      pastDate.setDate(today.getDate() - 30);
      const past30Days = pastDate.toISOString().split("T")[0];
      
      query += ` AND DATE(p.fecha_de_pedido) BETWEEN ? AND ?`;
      params.push(past30Days, todayDate);
  }
}
...
// Ruta para cambiar el estado de un pedido a inactivo
app.put('/pedidos/:id/inactivar', (req, res) => {
  const { id } = req.params;
  const query = `UPDATE pedido SET estado_bd = 'inactivo' WHERE idPedido = ?`;

  db.query(query, [id], (err, result) => {
      if (err) {
          console.error("Error al inactivar el pedido:", err);
          return res.status(500).send("Error al inactivar el pedido");
      }
      res.send("Pedido inactivado exitosamente");
  });
});
