...
function guardarCliente() {
    ...
    if (!nombre || !direccion || !telefono || !ciudad || isNaN(idBarrio)) {
...
function obtenerClientes(nombre = '', estado = 'activo') {
    axios.get(`http://localhost:3000/clientes`, {
        params: { nombre, estado }  // Pasar nombre y estado como parámetros de consulta
    })
    .then(respuesta => {
        const datos = respuesta.data;
        const table_cliente = document.getElementById("table_cliente");
        table_cliente.innerHTML = '';  // Limpiar la tabla antes de añadir
...
// Función para manejar la búsqueda por nombre y estado
function buscarClientes() {
    const nombre = document.getElementById('search-name').value;
    const estado = document.getElementById('search-status').value;
    obtenerClientes(nombre, estado);  // Llama a obtenerClientes con nombre y estado
}

// Cargar todos los clientes al inicio con estado "activo"
document.addEventListener("DOMContentLoaded", () => {
    obtenerClientes();
});
