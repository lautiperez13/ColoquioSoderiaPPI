...
function guardarPedido() {
    ...
    if (carrito.length === 0) {
        alert("No puedes guardar el pedido sin añadir productos al carrito.");
...
function eliminarPedido(idPedido) {
    axios.put(`http://localhost:3000/pedidos/${idPedido}/inactivar`)
        .then(() => {
            // alert("Pedido inactivado exitosamente");
            obtenerPedidos();  // Llama a la función que recarga los pedidos activos
        })
        .catch(error => {
...
function obtenerPedidos(nombre = '', estado = '', fechaFiltro = '') {
    const url = `http://localhost:3000/pedidos?nombre=${nombre}&estado=${estado}&fechaFiltro=${fechaFiltro}`;
    axios.get(url)
        .then(response => {
            ...
            pedidos.forEach(pedido => {
                ...
                row.innerHTML = `
                    ...
                    <td>${pedido.estado}</td>
                    <td>
                        <button onclick="mostrarProductos(${pedido.idPedido})">Detalle</button>
...
// Función para buscar pedidos usando los filtros
function buscarPedidos() {
    const nombre = document.getElementById('search-name').value;
    const estado = document.getElementById('search-status').value;
    const fechaFiltro = document.getElementById('search-date').value;
    obtenerPedidos(nombre, estado, fechaFiltro);  // Llama a obtenerPedidos con los valores de búsqueda
}

// Llama a obtenerPedidos sin parámetros para cargar todos los pedidos al inicio
document.addEventListener("DOMContentLoaded", () => {
    obtenerPedidos();
});
